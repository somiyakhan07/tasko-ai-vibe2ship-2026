...
import { getLocalDateString, Attachment } from '../types';
    ...
    try {
      if (actionProposal.type === 'proposeCreateTask') {
        ...
        await addEvent({
          ...
          date: formData.date || getLocalDateString(new Date()),
...
export const AICompanionModule: React.FC = () => {
  ...
  const handleAttachmentUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (isFirebaseConfigured && storage && profile?.uid) {
      try {
        const storageRef = ref(storage, `companion_attachments/${profile.uid}/${Date.now()}_${file.name}`);
        ...
        // Simulate a fast upload progress for fallback
    ...
    showToast(`Successfully attached "${file.name}"`, 'success');
  ...
  const handleToggleVoiceDictation = async () => {
    ...
    if (!SpeechRecognition) {
      showToast("Voice typing is not supported in this browser. Trying simulation...", "error");
    ...
    try {
      ...
      if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        try {
          ...
          console.error("Mic access error:", deviceErr);
          if (deviceErr.name === 'NotAllowedError' || deviceErr.name === 'PermissionDeniedError') {
            showToast("Microphone permission was denied. Please allow mic access in browser settings.", "error");
          } else if (deviceErr.name === 'NotFoundError' || deviceErr.name === 'DevicesError') {
            showToast("No microphone available. Please connect one and try again.", "error");
          } else {
            showToast("Unable to access microphone. Please check system settings.", "error");
          }
          return;
        }
      } else {
        showToast("Microphone is not supported in this environment.", "error");
        return;
      }
      ...
      rec.onresult = (event: any) => {
        ...
        if (transcript) {
          setInputMessage(prev => prev ? `${prev.trim()} ${transcript.trim()}` : transcript.trim());
      ...
      rec.onerror = (event: any) => {
        ...
        if (event.error === 'not-allowed') {
          ...
          showToast("No speech detected. Please speak clearly into the microphone.", "info");
        } else if (event.error === 'audio-capture') {
          showToast("Microphone is not capturing audio.", "error");
        } else {
  ...
  const simulateVoiceDictation = () => {
    setIsRecording(true);
    setListeningStatus('Listening (Simulated)...');
    const mockDictations = [
      "Review my high-priority tasks and drafts for today.",
      "Add a reminder to call team members at 2 PM.",
      "Summarize my recent project notes.",
      "Orchestrate a daily routine based on my habits."
    ];
    setTimeout(() => {
      const text = mockDictations[Math.floor(Math.random() * mockDictations.length)];
      setInputMessage(prev => prev ? `${prev.trim()} ${text}` : text);
      setIsRecording(false);
      showToast("Simulated voice captured successfully!", "success");
    }, 2500);
  };
  ...
  const handleSend = async (textToSend?: string) => {
    let finalMessage = (textToSend || inputMessage).trim();
    ...
    if (attachments.length > 0) {
      const attachmentsListStr = attachments.map(att => `📎 **[${att.name}](${att.url})** (${att.size})`).join('\n');
      if (finalMessage) {
        finalMessage = `${finalMessage}\n\n**Attached Files:**\n${attachmentsListStr}`;
      } else {
        finalMessage = `**Sent ${attachments.length} attached file(s):**\n${attachmentsListStr}`;
      }
    }
    ...
    try {
      await addMessageToChat(currentId, 'user', finalMessage, undefined, currentAttachments);
    } catch (err) {
      console.error(err);
    } finally {
      setIsThinking(false);
  ...
  return (
    <div id="ai-companion-module" className="flex h-full w-full bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      <section className="flex-1 flex flex-col h-full overflow-hidden bg-white dark:bg-slate-950">
        ...
        {/* Input Textbox Area */}
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 relative">
          
          {/* TOAST CONTAINER FOR VOICE/ERRORS */}
          <div className="absolute bottom-full left-0 right-0 z-50 pointer-events-none p-4 flex flex-col items-center gap-2">
            <AnimatePresence>
              {toasts.map((toast) => (
                <motion.div
                  key={toast.id}
                  ...
                  className={`pointer-events-auto flex items-center space-x-2 px-4 py-2.5 rounded-full shadow-lg text-xs font-semibold ${
                    toast.type === 'success'
                      ? 'bg-emerald-500 dark:bg-emerald-900 text-white shadow-emerald-500/10 border border-emerald-500/20'
                      : toast.type === 'error'
                      ? 'bg-rose-500 dark:bg-rose-900 text-white shadow-rose-500/10 border border-rose-500/20'
                      : 'bg-slate-900 dark:bg-slate-800 text-white shadow-slate-950/10 border border-slate-700/50'
                  }`}
                >
          ...
          <div className="max-w-3xl mx-auto space-y-2">
            ...
            {/* ATTACHMENT CHIPS ROW (ABOVE INPUT) */}
            <AnimatePresence>
                ...
                <div className="flex flex-wrap gap-2 pb-2 items-center">
                  {attachments.map((file) => (
                    <motion.div
                      ...
                      className="flex items-center space-x-1.5 pl-2.5 pr-1.5 py-1 bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-100 dark:border-indigo-900/50 rounded-lg text-2xs font-medium text-indigo-700 dark:text-indigo-350 shadow-sm"
                    >
                      <Paperclip className="w-3 h-3 text-indigo-500 flex-shrink-0" />
                      <span className="truncate max-w-[120px]">{file.name}</span>
                      <span className="text-[9px] text-slate-400 dark:text-slate-500">({file.size})</span>
                      <button
                        onClick={() => removeAttachment(file.id)}
                        className="p-0.5 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 rounded-md transition text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-350"
                      >
                        <X className="w-3 h-3" />
                      </button>
                    </motion.div>
                  ...
                  {isUploading && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, scale: 1 }}
                      exit={{ opacity: 0, scale: 0.9 }}
                      className="flex items-center space-x-2 px-2.5 py-1 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-[10px] font-medium text-slate-500 dark:text-slate-400 shadow-sm"
                    >
            ...
            <div className="relative flex items-center border border-slate-200 dark:border-slate-800 rounded-2xl bg-slate-50 dark:bg-slate-900/40 p-1.5 focus-within:ring-2 focus-within:ring-indigo-500/20 focus-within:border-indigo-500/50 transition-all">
              ...
              <AnimatePresence>
                {isRecording && (
                  <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="absolute inset-1 bg-slate-900 dark:bg-slate-950 rounded-xl flex items-center justify-between px-4 z-10"
                  >
                    <div className="flex items-center space-x-3">
                      ...
                      <span className="text-xs font-semibold text-rose-400 font-mono tracking-wide animate-pulse">
                        {listeningStatus}
                      </span>
                    </div>
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-rose-500 hover:bg-rose-600 text-white text-xs font-bold rounded-lg transition-all shadow-sm shadow-rose-500/10 cursor-pointer"
                    >
                      Stop Recording
                    </button>
                  </motion.div>
              ...
              {/* BUTTONS CONTAINER: ATTACHMENT | MICROPHONE | SEND */}
              <div className="flex items-center space-x-1.5 px-1.5 pb-0.5 shrink-0">
                {/* 📎 ATTACHMENT BUTTON */}
                <button
                  type="button"
                  ...
                  className={`p-2 rounded-xl flex items-center justify-center transition-all ${
                    isUploading 
                      ? 'bg-slate-100 dark:bg-slate-800 text-slate-400 cursor-not-allowed animate-pulse'
                      : 'text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800/80 cursor-pointer'
                  }`}
                  title="Attach file"
                >
                ...
                <button
                  ...
                  disabled={isThinking}
                  className={`p-2 rounded-xl flex items-center justify-center transition-all relative group ${
                    isRecording
                ...
                <button
                  type="button"
                  onClick={() => handleSend()}
                  disabled={(!inputMessage.trim() && attachments.length === 0) || isThinking}
                  className={`p-2 rounded-xl flex items-center justify-center transition-all ${
                    (inputMessage.trim() || attachments.length > 0) && !isThinking
                      ? 'bg-indigo-600 text-white hover:bg-indigo-700 shadow shadow-indigo-600/10 cursor-pointer scale-105 active:scale-95'
            ...
            <div className="flex justify-between items-center px-1.5 mt-2">
              ...
              <span className="text-[10px] text-slate-455 hidden sm:block font-mono">
